...
Public interface (matches app.py imports):
    VINA_AVAILABLE            : bool, whether the `vina` python package/CLI exists
...
NOTE: pose_pdb / complex_pdb now hold native PDBQT text (Vina's own output
format), not plain PDB — there's no PDBQT->PDB conversion step anymore.
Field names are kept as-is so app.py doesn't need any changes.
"""
...
# Meeko — AutoDock team's recommended SDF/RDKit-Mol -> PDBQT ligand
# preparation tool (proper torsion trees + charges). Replaces the earlier
# RDKit-PDB + obabel round-trip.
try:
...
class DockResult:
    ...
    pose_pdb: Optional[str] = None      # PDBQT text of the best pose (ligand only)
    complex_pdb: Optional[str] = None   # PDBQT text of protein+ligand complex
